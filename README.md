...
1. Press add button
1. Fill text fields with `http://download.nodepit.com/cyface/3.5`
1. Select --all sites-- in work with pulldown
1. Select Cyface KNIME Nodes
...
1. In the 'Package Explorer' tab expand the subproject `de.cyface.knime.targetplatform` and open (double-click) the `de.cyface.knime.targetplatform.target` file. Eclipse opens a new tab `Taget Definition`. Click the blue link `Set as Active Target Platform` at the top right.
